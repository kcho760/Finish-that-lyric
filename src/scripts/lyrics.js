...
class Lyrics {
  ...
  async getNewLyrics() {
    ...
    try {
      const response = await axios.get(chartUrl, { timeout: 10000 });
      ...
      const snippetResponse = await axios.get(snippetsUrl, { timeout: 5000 });
      const snippetData = snippetResponse.data;
      const snippets = snippetData.message.body.snippet.snippet_body.split("\n");
      const filteredSnippets = snippets.filter(snippet => !snippet.includes("This Lyrics is NOT for Commercial use") && !snippet.endsWith("..."));

      const randomIndexSnippet = Math.floor(Math.random() * filteredSnippets.length);
      const wordsToReplace = 1;
      const answerWords = [];
      const words = filteredSnippets[randomIndexSnippet].split(' ');
      const commonWords = ["I", "what", "a", "an", "the", "and", "that", "in", "you", "me", "is", "do", "can", "just", "of", "to", "for", "on", "with", "at", "by", "from", "up", "down", "out", "about", "my", "I'm", "oh", "ooh", "oooh", "ooooh", "oooooh", "ooooooh"];
      ...
      const promises = incorrectButtons.map(async (button) => {
        const response = await axios.get(`https://api.datamuse.com/words?ml=${answerWords[0].word}&max=100&min=4`);
